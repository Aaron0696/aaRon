...
* In some scales, there are conditions that if a participant answers less than half the number items, his total score should be coded as missing. This can be achieved by setting `NA.threshold = 0.5`, only calculate sum scores for those with *less than* 0.5 missingness. 
...
# only calculate for less than 0.2 missingness
...
`X` can be manuaaly defined by the user through the `unique.thres.fac` argument, defaults to 20.
